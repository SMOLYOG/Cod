...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? ImColor(255, 255, 255, 255) : hovered ? ImColor(137, 137, 137, 255) : ImColor(81, 81, 81, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ImColor(80, 80, 80, 255) : hovered ? ImColor(60, 60, 60, 255) : ImColor(40, 40, 40, 255), g.IO.DeltaTime * 6.f);
...
bool ImGui::ButtonEx(const char* label, const char* description, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImRect bb(pos + ImVec2(GetContentRegionMax().x - 330, 10), pos + ImVec2(GetContentRegionMax().x - 20, 40));
    const ImRect rect(pos, pos + ImVec2(GetContentRegionMax().x - style.WindowPadding.x, 50));
    ...
    auto Animation = ButtonAnimation.find(id);

    if (Animation == ButtonAnimation.end())
    {
        ButtonAnimation.insert({ id, Button_T() });
        Animation = ButtonAnimation.find(id);
    }


    //ImGui::GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, GetColorU32(Color::elements::background), Color::elements::rounding);

    //ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(Animation->second.background), Color::elements::rounding);

    ////ImGui::GetWindowDrawList()->AddText(ImVec2(rect.Min.x + 15, rect.Max.y - (50 + CalcTextSize(label).y) / 2), GetColorU32(Animation->second.text), label);

    ////ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (CalcTextSize("Button").x) / 2, bb.Max.y - (30 + CalcTextSize("Button").y) / 2), GetColorU32(Color::elements::text_active), label);
    //ImGui::GetWindowDrawList()->AddText(ImVec2(rect.Min.x + (CalcTextSize(label).x) / 2, bb.Max.y - (30 + CalcTextSize("Button").y) / 2), GetColorU32(Color::elements::text_active), label);
...
struct tab_state
{
    ImVec4 icon_col, text_col, desc_col;
};

static std::map<ImGuiID, tab_state> tab_anim;

bool ImGui::Tabs(const char* label, const char* desc, const char* icon, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.text_col = ImLerp(it_anim->second.text_col, selected ? main_color : hovered ? ImColor(137, 137, 137, 255) : ImColor(81, 81, 81, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.desc_col = ImLerp(it_anim->second.desc_col, selected ? ImColor(81, 81, 81, 255) : hovered ? ImColor(81, 81, 81, 255) : ImColor(81, 81, 81, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.icon_col = ImLerp(it_anim->second.icon_col, selected ? main_color : hovered ? ImColor(137, 137, 137, 255) : ImColor(81, 81, 81, 255), g.IO.DeltaTime * 6.f);
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(34, 34, 34), 0.f, 0, 1.f);

    ImGui::GetWindowDrawList()->AddText(Font::mainfont, 16, ImVec2(bb.Min.x + 55, bb.Max.y + style.FramePadding.y - 44), GetColorU32(it_anim->second.text_col), label);

    ImGui::GetWindowDrawList()->AddText(Font::mainfont_smaller, 14, ImVec2(bb.Min.x + 55, bb.Max.y + style.FramePadding.y - 27), GetColorU32(it_anim->second.desc_col), desc);

    ImGui::GetWindowDrawList()->AddText(Font::icons, 20, ImVec2(bb.Min.x + 16, bb.Max.y + style.FramePadding.y - 39), GetColorU32(it_anim->second.icon_col), icon);
...
static std::map<ImGuiID, subtab_state> sub_anim;

bool ImGui::Subtabs(const char* label, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    auto it_anim = sub_anim.find(id);

    if (it_anim == sub_anim.end())
    {
        sub_anim.insert({ id, subtab_state() });
        it_anim = sub_anim.find(id);
    }

    it_anim->second.text = ImLerp(it_anim->second.text, selected ? main_color : hovered ? ImColor(137, 137, 137, 255) : ImColor(81, 81, 81, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? main_color : ImColor(34, 34, 34, 255), g.IO.DeltaTime * 6.f);

    // Render
    RenderNavHighlight(bb, id);
    window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Max.y), bb.Max, GetColorU32(it_anim->second.background), 1.f);
...
static std::map<ImGuiID, KeyBind_Ta> KeyBindAnimationa;
...
bool ImGui::Keybind(const char* label, int* key)
{
    ...
    else if (*key != 0 && *key >= 200 && g.ActiveId != id) {     
        strcpy_s(buf_display, ResolveString(controllerButton).c_str());
    }
    ...
    auto it_anim = KeyBindAnimationa.find(id);

    if (it_anim == KeyBindAnimationa.end())
    {
        KeyBindAnimationa.insert({ id, KeyBind_Ta() });
        it_anim = KeyBindAnimationa.find(id);
    }

    it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? ImColor(255, 255, 255, 255) : hovered ? ImColor(116, 117, 120, 255) : ImColor(91, 92, 94, 255), ImGui::GetIO().DeltaTime * 6.f);

    window->DrawList->AddText(rect.Min + ImVec2(0, 5), GetColorU32(it_anim->second.text), label);

    window->DrawList->AddRectFilled(key_rect.Min + ImVec2(CalcItemWidth() - 21, -1), key_rect.Max - ImVec2(2, 12), ImColor(40, 40, 40), 4.f);

    PushFont(Font::interbinoshift);
    ...
    if (g.ActiveId == id) {
        ...
        controllerButton = ResolveButton();
...
bool ImGui::CheckBoxP(const char* label, bool* v, const ImVec2& size_arg)
{
    ...
    CheckBox2_T& state = CheckBox2Animation[id];
...
bool ImGui::SubTab(bool selected, const char* label, const char* description, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    RenderTextColor(Font::lexend_bold, bb.Min + ImVec2(10, 0), bb.Max, GetColorU32(state.text), label, ImVec2(0.0, 0.3));
   /// RenderTextColor(Font::icomoon, bb.Min, bb.Min + ImVec2(size.y, size.y), GetColorU32(state.icon), icon, ImVec2(0.5, 0.5));
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, main_color, style.ScrollbarRounding);
...
struct check_state
{
    ImVec4 background, text, rect_col;
    float alpha_mark;
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float w = GetContentRegionMax().x - style.WindowPadding.x - 25;
    ...
    it_anim->second.alpha_mark = ImLerp(it_anim->second.alpha_mark, *v ? 1.f : hovered ? 0.39f : 0.f, g.IO.DeltaTime * 8.f);
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? main_color : hovered ? ImColor(80, 80, 80, 255) : ImColor(40, 40, 40, 255), g.IO.DeltaTime * 8.f);
    it_anim->second.text = ImLerp(it_anim->second.text, *v ? ImColor(255, 255, 255, 255) : hovered ? ImColor(116, 117, 120, 255) : ImColor(91, 92, 94, 255), g.IO.DeltaTime * 8.f);
    ...
    RenderCheckMark(GetWindowDrawList(), check_bb.Min + ImVec2(square_sz / 2, square_sz / 2) / 2, GetColorU32(ImGuiCol_Text, it_anim->second.alpha_mark), square_sz / 2);
...
bool ImGui::Checkbox(const char* label, const char* description, bool* v)
{
    ...
    ImRect rect(pos, pos + ImVec2(w, 50));
    ...
    CheckBox_T& state = CheckBoxAnimation[id];
...
static std::map<ImGuiID, begin_state> animbegincom;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
{
    ...
    SetCursorPos(GetCursorPos() + ImVec2(0, 3));
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = ((GetContentRegionMax().x - style.WindowPadding.x - 23));
    const float y_size = 26;
    ...
    auto it_anim = animbegincom.find(id);

    if (it_anim == animbegincom.end())
    {
        animbegincom.insert({ id, begin_state() });
        it_anim = animbegincom.find(id);
    }
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? ImColor(255, 255, 255, 255) : hovered ? ImColor(116, 117, 120, 255) : ImColor(91, 92, 94, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 32) + 15 : 0.f, g.IO.DeltaTime * 12.f);

    GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, ImColor(40, 40, 40), 4.f);

    GetWindowDrawList()->AddText(ImVec2(window->DC.CursorPos.x, rect.Min.y + (22 - CalcTextSize(label).y) / 2), GetColorU32(it_anim->second.text), label);

    PushClipRect(rect.Min, rect.Max, true);
    GetWindowDrawList()->AddText(ImVec2(rect.Min.x + ((y_size - CalcTextSize(label).y) / 2) + 2, rect.Min.y + (y_size - CalcTextSize(label).y) / 2), ImColor(255, 255, 255), preview_value);
    PopClipRect();

    GetWindowDrawList()->AddRectFilled(rect.Max - ImVec2(y_size, y_size), rect.Max, GetColorU32(it_anim->second.background), 4.f);
    GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(rect.Min.x, rect.Max.y - y_size), rect.Max, ImColor(40, 40, 40, 0), ImColor(40, 40, 40), ImColor(40, 40, 40), ImColor(40, 40, 40, 0), 4.f);

    PushFont(Font::icons);
    GetWindowDrawList()->AddText(ImVec2(bb.Max.x - (y_size + CalcTextSize("i").y) / 2, bb.Min.y + (y_size - CalcTextSize("i").y) / 2 - 5), GetColorU32(it_anim->second.text), "K");
    PopFont();
...
bool ImGui::Comboa(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;
...
{
    const bool value_changed = Comboa(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items);
    return value_changed; 
}
...
bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items)
{
    ...
    bool value_changed = Comboa(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items);
    return value_changed;
}
...
static std::map<ImGuiID, slider_state> animz;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() + 30;
    ...
    auto it_anim = animz.find(id);

    if (it_anim == animz.end())
    {
        animz.insert({ id, slider_state() });
        it_anim = animz.find(id);
    }

    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? ImColor(255, 255, 255, 255) : hovered ? ImColor(116, 117, 120, 255) : ImColor(91, 92, 94, 255), g.IO.DeltaTime * 8.f);

    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 8), frame_bb.Max - ImVec2(5, 6), ImColor(40, 40, 40, 255), 100);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        it_anim->second.slow = ImLerp(it_anim->second.slow, grab_bb.Min.x + 5 - frame_bb.Min.x + 10, g.IO.DeltaTime * 20.f);
    ...
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(frame_bb.Min - ImVec2(0, 22), frame_bb.Max - ImVec2(0, 20), value_buf, value_buf_end, NULL, ImVec2(0.98f, 0.5f));
...
bool ImGui::SliderScalar(const char* label, const char* description, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    Slider_T& state = SliderAnimation[id];
...
bool ImGui::RangeSliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v1, float* v2, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ...
    Range_T& state = RangeAnimation[id];
...
bool ImGui::RangeSliderFloat(const char* label, const char* description, float* v1, float* v2, float v_min, float v_max, const char* display_format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)return false;
    ...
    Range_T& state = RangeFloatAnimation[id];
...
bool ImGui::QuandScalar(const char* label, const char* description, ImGuiDataType data_type, void* p_data0, void* p_data1, const void* p_min0, const void* p_max0, const void* p_min1, const void* p_max1, const char* format0, const char* format1, ImGuiSliderFlags flags)
{
    ...
    auto it_anim = QuandAnimation.find(id);

    if (it_anim == QuandAnimation.end())
    {
        QuandAnimation.insert({ id, Quand_T() });
        it_anim = QuandAnimation.find(id);
    }
...
bool ImGui::QuandFloat(const char* label, const char* description, float* v0, float* v1, float v_min0, float v_max0, float v_min1, float v_max1, const char* format0, const char* format1, ImGuiSliderFlags flags)
{
    return QuandScalar(label, description, ImGuiDataType_Float, v0, v1, &v_min0, &v_max0, &v_min1, &v_max1, format0, format1, flags);
}

bool ImGui::QuandInt(const char* label, const char* description, int* v0, int* v1, int v_min0, int v_max0, int v_min1, int v_max1, const char* format0, const char* format1, ImGuiSliderFlags flags)
{
    return QuandScalar(label, description, ImGuiDataType_S32, v0, v1, &v_min0, &v_max0, &v_min1, &v_max1, format0, format1, flags);
}


bool ImGui::SliderFloat(const char* label, const char* description, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalar(label, description, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}

bool ImGui::SliderInt(const char* label, const char* description, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    return ImGui::SliderScalar(label, description, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min, const void* p_clamp_max)
{
    ...
    if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags))
    {
        ...
        // Only mark as ImGui if new value is different
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(-1, 30), flags, callback, user_data);
}
...
bool ImGui::InputTextWithHint(const char* label, const char* hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, hint, buf, (int)buf_size, ImVec2(640, 30), flags, callback, user_data);
}
...
static std::map<ImGuiID, input_state> animinput;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    auto it_anim = animinput.find(id);

    if (it_anim == animinput.end())
    {
        animinput.insert({ id, input_state() });
        it_anim = animinput.find(id);
    }

    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(40, 40, 40, 255), 4.f);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            ImU32 col = GetColorU32(is_displaying_hint ? ImVec4(0.6f, 0.6f, 0.6f, 1.f) : ImVec4(1.0f, 1.0f, 1.0f, 1.f));
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second.slow, 8), total_bb.GetBL() - ImVec2(-it_anim->second.slow, 8), ImColor(255, 255, 255, 255));
...
bool ImGui::icon_box(const char* icon, ImVec2 size, ImU32 color_bg, ImU32 color_icon, ImU32 color_border)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    IconBox_T& state = IndexAnimation[id];
...
bool ImGui::color_button(const char* name, ImVec2 size, ImU32 color_bg)
{
    ...
    IconBox_T& state = ColorAnimation[id];
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(w - 3, 4), window->DC.CursorPos + size + ImVec2(w - 6, -5));
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        window->DC.CursorPos = ImVec2(pos.x + button_offset_x - 25, pos.y);
        ...
        PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(22, 24, 23)));
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel)) GetWindowDrawList()->AddText(window->DC.CursorPos - ImVec2(w_button, -1), GetColorU32(ImGuiCol_Text), label);

    ImFormatString(buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X", ImClamp(i[0], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[2], 0, 255));
    // GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(GetWindowContentRegionMax().x - 140, 1), GetColorU32(text::text), buf);
    if (!(flags & ImGuiColorEditFlags_NoLabel))
    {
        PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(66, 67, 72)));
        ImGui::RenderTextClipped(window->DC.CursorPos + ImVec2(GetWindowContentRegionMax().x - 400, 0), window->DC.CursorPos + ImVec2(GetWindowContentRegionMax().x - 80, 19), buf, NULL, NULL, ImVec2(0.94f, 0.5f));
        PopStyleColor();
    }
...
bool ImGui::ColorEdit4(const char* label, const char* description, float col[4], ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    ColorEdit_T& state = ColorEditAnimation[ImGui::GetID(label)];
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (state.active)
        {
            ...
            Begin("picker", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize);
            {
                ...
                //ImGui::Checkbox("Rainbow", "Rainbow Skeleton", &Aimbot::Enable);
                //ImGui::SetCursorPosY(GetCursorPosY() + 80);
                ...
                if (InputTextEx("J", "HEX COLOR", buf, IM_ARRAYSIZE(buf), ImVec2(!(flags & ImGuiColorEditFlags_NoAlpha) ? 128 : 118, 35), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
                {
                    value_changed = true;
                    char* p = buf;
                ...
                for (int i = 0; i < color_x.size(); i++)
                {
                    ...
                    if (ImGui::color_button(name_box.c_str(), ImVec2(17, 17), ImColor(color_x[i], color_y[i], color_z[i], color_a[i])))
                    {
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    Picker_T& state = ColorPickerAnimation[ImGui::GetID(label)];
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
    ...
    if (g.LogEnabled && !(flags & ImGuiTreeNodeFlags_NoAutoOpenOLog_) && (window->DC.TreeDepth - g.LogDepthRef) < g.LogDepthToExpand)
        is_open = true;
...
static std::map<ImGuiID, select_state> animselet;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    auto it_anim = animselet.find(id);

    if (it_anim == animselet.end())
    {
        animselet.insert({ id, select_state() });
        it_anim = animselet.find(id);
    }
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? main_color : hovered ? ImColor(116, 117, 120, 255) : ImColor(91, 92, 94, 255), g.IO.DeltaTime * 8.f);
