...
struct Selector_T
{
    ...
    float alpha;
}; inline std::map<ImGuiID, Selector_T> SelectorAnimation;

struct Button_T
{
    ImVec4 background, text;
}; inline std::map<ImGuiID, Button_T> ButtonAnimation;
...
struct KeyBind_T
{
    ...
    bool active, hovered;
    float alpha = 0.f;
}; inline std::map<ImGuiID, KeyBind_T> KeyBindAnimation;

struct CheckBox2_T
{
    ImVec4 background;
}; inline std::map<ImGuiID, CheckBox2_T> CheckBox2Animation;

struct Tab_T
{
    ImVec4 background, text, icon, shadow;
    float bg_alpha;
}; inline std::map<ImGuiID, Tab_T> TabAnimation;

struct CheckBox_T
{
    ImVec4 background, text;
    float alpha, mark_pos;
}; inline std::map<ImGuiID, CheckBox_T> CheckBoxAnimation;

struct Slider_T
{
    float slow, blick;
    ImVec4 text;
}; inline std::map<ImGuiID, Slider_T> SliderAnimation;

struct BeginCombo_T
{
    float open, alpha, combo_size, shadow_opticaly, arrow_roll;
    bool opened_combo = false, hovered = false;
    ImVec4 background, text, outline;
}; inline std::map<ImGuiID, BeginCombo_T> BeginComboAnimation;

struct Selectable_T
{
    ImVec4 text;
    float opticaly;
}; inline std::map<ImGuiID, Selectable_T> SelecatableAnimation;

struct Range_T
{
    ImVec4 text;
    float slow1, slow2, alpha_text, text_offset;
}; inline std::map<ImGuiID, Range_T> RangeAnimation, RangeFloatAnimation;

struct Quand_T
{
    float slow0, slow1, tickness, move_yy, move_xx;
    int move_y, move_x;
}; inline std::map<ImGuiID, Quand_T> QuandAnimation;

struct IconBox_T
{
    float line;
}; inline std::map<ImGuiID, IconBox_T> IndexAnimation, ColorAnimation;
...
struct ColorEdit_T
{
    ...
    bool hovered, active;
}; inline std::map<ImGuiID, ColorEdit_T> ColorEditAnimation;
...
struct Picker_T
{
    ...
    float circle;
    ImVec2 circle_move;
}; inline std::map<ImGuiID, Picker_T> ColorPickerAnimation;
...
namespace ImGui
{
    ...
    IMGUI_API void          SetNextWindowContentSize(const ImVec2& size);                               // set next window content size (~ scrollable ClientNumber area, which enforce the range of scrollbars). Not including window decorations (title bar, menu bar, etc.) nor WindowPadding. set an axis to 0.0f to leave it automatic. call before Begin()
    ...
    IMGUI_API bool          Subtabs(const char* label, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags = 0);
    ...
    IMGUI_API bool          SubTab(bool selected, const char* label, const char* description, const ImVec2& size_arg);
    ...
    IMGUI_API bool          Checkbox2(const char* label, const char* description, bool* v);
    ...
    IMGUI_API bool          Comboa(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int popup_max_height_in_items = -1);
    ...
    //   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
    IMGUI_API bool          SliderFloat(const char* label, const char* description, float* v, float v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags = 0);

    IMGUI_API bool          SliderInt(const char* label, const char* description, int* v, int v_min, int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
    ...
    IMGUI_API bool          SliderScalar(const char* label, const char* description, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format = NULL, ImGuiSliderFlags flags = 0);
    ...
    IMGUI_API bool          RangeSliderFloat(const char* label, const char* description, float* v1, float* v2, float v_min, float v_max, const char* display_format = "(%.3f, %.3f)", float power = 1.0f);
    ...
    IMGUI_API bool          QuandScalar(const char* label, const char* description, ImGuiDataType data_type, void* p_data0, void* p_data1, const void* p_min0, const void* p_max0, const void* p_min1, const void* p_max1, const char* format0 = "%d", const char* format1 = "%d", ImGuiSliderFlags flags = 0);
    IMGUI_API bool          QuandFloat(const char* label, const char* description, float* v0, float* v1, float v_min0 = 0.0f, float v_max0 = 0.0f, float v_min1 = 0.0f, float v_max1 = 0.0f, const char* format0 = "%d", const char* format1 = "%d", ImGuiSliderFlags flags = 0);
    IMGUI_API bool          QuandInt(const char* label, const char* description, int* v0, int* v1, int v_min0 = 0, int v_max0 = 0, int v_min1 = 0, int v_max1 = 0, const char* format0 = "%d", const char* format1 = "%d", ImGuiSliderFlags flags = 0);
    // Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little color square that can be left-clicked to open a picker, and right-clicked to open an option menu.)
    // - Note that in C++ a 'float v[X]' function argument is the _same_ as 'float* v', the array syntax is just a way to document the number of elements that are expected to be accessible.
    // - You can pass the address of a first float element out of a contiguous structure, e.g. &myvector.x
    
    IMGUI_API bool          icon_box(const char* icon, ImVec2 size, ImU32 color_bg, ImU32 color_icon, ImU32 color_border);
    ...
    IMGUI_API bool          ColorEdit4(const char* label, const char* description, float col[4], ImGuiColorEditFlags flags = 0);
    ...
    // - DLL users: heaps and globals are not shared across DLL boundaries! You will need to call SetCurrentCont
    // 
    // for more details.
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_NoAutoOpenOLog_ = 1 << 4,   // Don't automatically and temporarily open node when Logging is active (by default logging will automatically open tree nodes)
    ...
    ImGuiTreeNodeFlags_CollapsingHeader = ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_NoTreePushOnOpen | ImGuiTreeNodeFlags_NoAutoOpenOLog_,
...
enum ImGuiCol_
{
    ...
    ImGuiCol_TabDimmedSelectedOverline,//..horizontal overline, when tab-bar is unfocused & tab is selected// TabItem in a TabBar
    ImGuiCol_TabHovered,
    ImGuiCol_TabActive,
